...
int gear = 1;
int gear_ratio[5] = {100, 70, 50, 40, 32};
...
unsigned long previous_time_sensors = 0;
...
void loop()  { 
  ...
  if (current_time - previous_time_sensors > SensorInterval)
  {
    ...
    previous_time_sensors = current_time;
  }
  ...
  if (BrakePedal > 0.8 && gear == 1)
  {
    ...
    {
      RPM = RPM + 30*GasPedal - 5*RPM/7000 - 5;
  ...
  else
  {
    ...
    if (current_time - previous_rpm_update > RPM_update_interval)
    {
      RPM = RPM + (2.5*GasPedal - 6*BrakePedal) * float(gear_ratio[gear-1])/50.0 - 0.25 - random(-200, 200)/Speed;
  ...
  if(current_time - previous_time_tone > tone_interval)
  {
    ...
    */
    ...
    if (RPM < 1000 && Fuelempty && !Horn) noTone(Speaker); else tone(Speaker, int(tonefreq));
  ...
  if(current_time - previousMillisup > RPMinterval && current_time - previousMillisdown > RPMinterval/2)
  {
    if(RPM >= RPMmin) RPMstate = HIGH;
    if (RPM < 4500 && !Horn) digitalWrite(Speaker, RPMstate);
    digitalWrite(RPMGauge, RPMstate);
    previousMillisup = current_time;
  }
  if(current_time - previousMillisdown > RPMinterval && current_time - previousMillisup > RPMinterval/2)
  {
    RPMstate = LOW;
    if (RPM < 4500 && !Horn) digitalWrite(Speaker, RPMstate);
    digitalWrite(RPMGauge, RPMstate);
